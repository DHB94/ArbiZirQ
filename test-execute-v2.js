// Test script for flash loan trade execution - Version 2

const fetch = require('node-fetch');

async function testExecuteEndpoint() {
  console.log('🧪 Testing flash loan trade execution (V2)...');
  ...
  try {
    ...
    // Test with GUD execution
    console.log('\nTesting GUD execution directly...');
    const gudRequest = { 
      ...testRequest, 
      dryRun: false,
      // Use a smaller amount to avoid potential bigint issues
      sizeDollar: 100
    };
    
    const gudResponse = await fetch('http://localhost:3000/api/execute', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Use-GUD': 'true' // Custom header to force GUD execution
      },
      body: JSON.stringify(gudRequest)
    });
    
    const gudResult = await gudResponse.json();
    console.log('GUD execution response status:', gudResponse.status);
    console.log('GUD execution result:', JSON.stringify(gudResult, null, 2));
    
    if (!gudResponse.ok) {
      console.error('❌ GUD execution test failed');
      return;
    }
    
    console.log('✅ GUD execution test successful');
